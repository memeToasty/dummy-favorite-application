...
$(OUTPUT).cia:  $(OUTPUT).3dsx
	...
	@bannertool makesmdh -i "../$(META)/icon.png" -s "$(TARGET)" -l "$(APP_DESCRIPTION)" -p "$(APP_AUTHOR)" -o "../$(META)/icon.bin" \
	...
	@makerom -f cia -target t -exefslogo -o $(OUTPUT).cia -elf $(OUTPUT).elf -rsf ../$(META)/rominfo.rsf -icon ../$(META)/icon.bin
